...
opin system heartbeat last
opin logs --follow
opin config get agents.defaults.heartbeat
opin channels status --probe
```
...
```bash
opin config get agents.defaults.heartbeat.activeHours
opin config get agents.defaults.heartbeat.activeHours.timezone
opin config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
opin cron list
opin logs --follow
```
