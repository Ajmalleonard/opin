...
- Jobs persist under `~/.opin/cron/` so restarts don’t lose schedules.
...
opin cron list
opin cron run <job-id>
opin cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.opin/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `opin cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Opin defaults to `announce`.
...
- Job store: `~/.opin/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.opin/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
opin cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
opin cron edit <jobId> --agent ops
opin cron edit <jobId> --clear-agent
```
...
```bash
opin cron run <jobId>
opin cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
opin cron edit <jobId> \
...
```bash
opin system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`opin system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OPIN_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Opin applies exponential retry backoff for recurring jobs after consecutive errors:
