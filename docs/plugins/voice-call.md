...
read_when:
  - You want to place an outbound voice call from Opin
...
Voice calls for Opin via a plugin. Supports outbound notifications and
...
- Use `opin voicecall ...` or the `voice_call` tool
...
```bash
opin plugins install ./extensions/voice-call
