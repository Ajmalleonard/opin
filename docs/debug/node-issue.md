...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Opin
...
Running Opin via Node with `tsx` fails at startup with:

```
[opin] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `opin status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch opin.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node opin.mjs status` works on Node 25.
