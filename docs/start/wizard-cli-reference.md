...
read_when:
  - You need detailed behavior for opin onboard
...
This page is the full reference for `opin onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.opin/opin.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `opin doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `opin pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `opin health`.
    - `opin status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.opin/.env` so launchd can read it.
...
- OAuth credentials: `~/.opin/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.opin/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`opin onboard`](/cli/onboard)
