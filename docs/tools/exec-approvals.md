...
- **gateway host** → `opin` process on the gateway machine
...
`~/.opin/exec-approvals.json` directly.

CLI: `opin approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
