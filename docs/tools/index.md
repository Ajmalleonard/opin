---
summary: "Agent tool surface for Opin (browser, canvas, nodes, message, cron) replacing legacy `opin-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `opin-*` skills
title: "Tools"
---

# Tools (Opin)

Opin exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `opin-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `opin.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Opin logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`opin node run`).
...
- Requires a Brave API key (recommended: `opin configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Opin-managed browser.
...
- Quick smoke: `opin nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `opin gateway` restart in-place)
