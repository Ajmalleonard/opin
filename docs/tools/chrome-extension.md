---
summary: "Chrome extension: let Opin drive your existing Chrome tab"
read_when:
...
The Opin Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate opin-managed Chrome profile.
...
Opin then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Opin release (npm package) as static files. There is no separate “build” step.

After upgrading Opin:

- Re-run `opin browser extension install` to refresh the installed files under your Opin state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Opin ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Opin to control.
...
- `ON`: attached; Opin can drive that tab.
...
Debugging: `opin sandbox explain`
...
`opin browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `opin browser extension install` first to copy the extension to a stable location under your Opin state directory.
...
- **This is not isolated** like the dedicated opin-managed profile.
