...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `opin sandbox explain`.
...
Legacy `agent.*` configs are migrated by `opin doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=opin-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPIN_STATE_DIR:-$HOME/.opin}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
