---
summary: "Direct `opin agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `opin agent` (direct agent runs)

`opin agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `opin message --target`).
...
```bash
opin agent --to +25512345678 --message "status update"
...
opin agent --to +25512345678 --message "Summon reply" --deliver
