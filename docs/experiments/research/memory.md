---
summary: "Research notes: offline memory system for Opin workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.opin/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Opin integration
...
Target: Opin-style workspace (`agents.defaults.workspace`, default `~/.opin/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.opin/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Opin to see every session.
...
```
~/.opin/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Opin**, but keep a separable core library.

### Why integrate into Opin?
...
- You want the agent itself to call the tools:
  - `opin memory recall "…" --k 25 --since 30d`
  - `opin memory reflect --since 7d`
...
Pragmatic take for `~/.opin/workspace`:
