...
service via the external `opin` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`OPIN_PROFILE`; legacy `com.opin.*` is supported).
