---
summary: "Opin on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Opin on a Raspberry Pi
  - Running Opin on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Opin on Raspberry Pi

## Goal

Run a persistent, always-on Opin Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Opin features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled opin

# Enable if not
sudo systemctl enable opin

# Start on boot
sudo systemctl start opin
```
...
```bash
# Check logs
journalctl -u opin --no-pager -n 100

# Common fix: rebuild
cd ~/opin  # if using hackable install
npm run build
sudo systemctl restart opin
```
