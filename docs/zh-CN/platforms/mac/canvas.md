...
- `~/Library/Application Support/Opin/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `opin-canvas://<session>/<path>`

示例：

- `opin-canvas://main/` → `<canvasRoot>/main/index.html`
- `opin-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `opin-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
opin nodes canvas present --node <id>
opin nodes canvas navigate --node <id> --url "/"
opin nodes canvas eval --node <id> --js "document.title"
opin nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__opin__/a2ui/
```
...
opin nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
opin nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "opin://agent?message=Review%20this%20design";
```
