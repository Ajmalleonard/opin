...
summary: 设置指南：在保持最新的同时保持你的 Opin 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.opin/workspace`（工作区）+ `~/.opin/opin.json`（配置）。
...
- **配置：** `~/.opin/opin.json`（JSON/JSON5 格式）
- **工作区：** `~/.opin/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm opin setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **Opin.app**（菜单栏）。
...
- 运行 `opin setup`，然后 `opin channels login`，然后手动启动 Gateway 网关（`opin gateway`）。
...
- **配对允许列表**：`~/.opin/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.opin/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.opin/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.opin/workspace` 和 `~/.opin/` 保持为"你的东西"；不要将个人提示/配置放入 `opin` 仓库。
