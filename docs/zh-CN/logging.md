...
所有日志配置都在 `~/.opin/opin.json` 的 `logging` 下。
...
- Opin 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `opin plugins enable diagnostics-otel` 启用插件。
...
- `opin.tokens`（计数器，属性：`opin.token`、`opin.channel`、`opin.provider`、`opin.model`）
- `opin.cost.usd`（计数器，属性：`opin.channel`、`opin.provider`、`opin.model`）
- `opin.run.duration_ms`（直方图，属性：`opin.channel`、`opin.provider`、`opin.model`）
- `opin.context.tokens`（直方图，属性：`opin.context`、`opin.channel`、`opin.provider`、`opin.model`）

消息流：

- `opin.webhook.received`（计数器，属性：`opin.channel`、`opin.webhook`）
- `opin.webhook.error`（计数器，属性：`opin.channel`、`opin.webhook`）
- `opin.webhook.duration_ms`（直方图，属性：`opin.channel`、`opin.webhook`）
- `opin.message.queued`（计数器，属性：`opin.channel`、`opin.source`）
- `opin.message.processed`（计数器，属性：`opin.channel`、`opin.outcome`）
- `opin.message.duration_ms`（直方图，属性：`opin.channel`、`opin.outcome`）

队列 + 会话：

- `opin.queue.lane.enqueue`（计数器，属性：`opin.lane`）
- `opin.queue.lane.dequeue`（计数器，属性：`opin.lane`）
- `opin.queue.depth`（直方图，属性：`opin.lane` 或 `opin.channel=heartbeat`）
- `opin.queue.wait_ms`（直方图，属性：`opin.lane`）
- `opin.session.state`（计数器，属性：`opin.state`、`opin.reason`）
- `opin.session.stuck`（计数器，属性：`opin.state`）
- `opin.session.stuck_age_ms`（直方图，属性：`opin.state`）
- `opin.run.attempt`（计数器，属性：`opin.attempt`）
...
- `opin.webhook.error`
  - `opin.channel`、`opin.webhook`、`opin.chatId`、`opin.error`
- `opin.message.processed`
  - `opin.channel`、`opin.outcome`、`opin.chatId`、`opin.messageId`、`opin.sessionKey`、`opin.sessionId`、`opin.reason`
- `opin.session.stuck`
  - `opin.state`、`opin.ageMs`、`opin.queueDepth`、`opin.sessionKey`、`opin.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `opin doctor`。
