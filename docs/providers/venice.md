---
summary: "Use Venice AI privacy-focused models in Opin"
read_when:
  - You want privacy-focused inference in Opin
...
```bash
opin onboard --auth-choice venice-api-key
```
...
```bash
opin chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Opin shows all available Venice models. Pick based on your needs:
...
```bash
opin models set venice/claude-opus-45
opin models set venice/llama-3.3-70b
```

List all available models:

```bash
opin models list | grep venice
```

## Configure via `opin configure`

1. Run `opin configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Opin automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
opin chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
opin chat --model venice/claude-opus-45

# Use uncensored model
opin chat --model venice/venice-uncensored

# Use vision model with image
opin chat --model venice/qwen3-vl-235b-a22b

# Use coding model
opin chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `opin models list` to see currently available models. Some models may be temporarily offline.
