---
summary: "Use MiniMax M2.1 in Opin"
read_when:
  - You want MiniMax models in Opin
...
```bash
opin plugins enable minimax-portal-auth  # skip if already loaded.
opin gateway restart  # restart if gateway is already running
opin onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/opin/opin/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `opin.json`:
...
- Use `opin models list` and `opin models set minimax/MiniMax-M2.1` to switch.
...
- Running `opin configure` and selecting **MiniMax M2.1**, or
