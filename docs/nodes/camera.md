...
Opin supports **camera capture** for agent workflows:
...
```bash
opin nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
opin nodes camera snap --node <id> --facing front
opin nodes camera clip --node <id> --duration 3000
opin nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`opin.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `opin` CLI to invoke camera commands on the macOS node.

Examples:

```bash
opin nodes camera list --node <id>            # list camera ids
opin nodes camera snap --node <id>            # prints MEDIA:<path>
opin nodes camera snap --node <id> --max-width 1280
opin nodes camera snap --node <id> --delay-ms 2000
opin nodes camera snap --node <id> --device-id <id>
opin nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
opin nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
opin nodes camera clip --node <id> --device-id <id>
opin nodes camera clip --node <id> --no-audio
```

Notes:

- `opin nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
opin nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
