...
read_when:
  - You want to understand what “context” means in Opin
...
“Context” is **everything Opin sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Opin-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Opin builds the system prompt

The system prompt is **Opin-owned** and rebuilt each run. It includes:
...
By default, Opin injects a fixed set of workspace files (if present):
