...
- `/usage cost` in chats: local cost summary aggregated from Opin session logs.
- CLI: `opin status --usage` prints a full per-provider breakdown.
- CLI: `opin channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
