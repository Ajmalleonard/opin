...
```bash
opin plugins install @opin/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
opin plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Opin" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `opin pairing list nextcloud-talk`
  - `opin pairing approve nextcloud-talk <CODE>`
