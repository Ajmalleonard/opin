...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Opin to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
opin plugins install @opin/feishu
```

Local checkout (when running from a git repo):

```bash
opin plugins install ./extensions/feishu
```
...
If you just installed Opin, run the wizard:
...
2. Configuring app credentials in Opin
...
1. You already ran `opin channels add` for Feishu
2. The gateway is running (`opin gateway status`)
...
```bash
opin pairing approve feishu <CODE>
```
...
2. Run `opin logs --follow` and look for `chat_id`
...
2. Run `opin logs --follow` and look for `open_id`
...
| Command                | Description                   |
| ---------------------- | ----------------------------- |
| `opin gateway status`  | Show gateway status           |
| `opin gateway install` | Install/start gateway service |
| `opin gateway stop`    | Stop gateway service          |
| `opin gateway restart` | Restart gateway service       |
| `opin logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `opin gateway status`
6. Check logs: `opin logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "opin-fan",
        workspace: "/home/user/opin-fan",
        agentDir: "/home/user/.opin/agents/opin-fan/agent",
      },
      {
        id: "opin-xi",
        workspace: "/home/user/opin-xi",
        agentDir: "/home/user/.opin/agents/opin-xi/agent",
      },
    ],
  },
  bindings: [
