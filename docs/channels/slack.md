...
3. Set tokens for Opin and start the gateway.
...
7. Slash Commands → create `/opin` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
Opin can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Opin falls back to the user token.
...
```json
{
  "display_information": {
    "name": "Opin",
    "description": "Slack connector for Opin"
  },
  "features": {
    "bot_user": {
      "display_name": "Opin",
    ...
    "slash_commands": [
      {
        "command": "/opin",
        "description": "Send a message to Opin",
        "should_escape": false
      }
    ]
  },
...
By default, Opin replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Opin infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `opin pairing approve slack <code>`.
...
- On startup, Opin resolves channel/user names in allowlists to IDs (when tokens allow)
