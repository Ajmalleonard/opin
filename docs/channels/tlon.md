...
Tlon is a decentralized messenger built on Urbit. Opin connects to your Urbit ship and can
...
```bash
opin plugins install @opin/tlon
```

Local checkout (when running from a git repo):

```bash
opin plugins install ./extensions/tlon
```
...
Use these with `opin message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Opin replies in-thread.
