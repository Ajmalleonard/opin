...
Matrix is an open, decentralized messaging protocol. Opin connects as a Matrix **user**
...
```bash
opin plugins install @opin/matrix
```

Local checkout (when running from a git repo):

```bash
opin plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `opin plugins install @opin/matrix`
   - From a local checkout: `opin plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Opin calls the same
     login endpoint, stores the access token in `~/.opin/credentials/matrix/credentials.json`,
...
- On first connection, Opin requests device verification from your other sessions.
...
`~/.opin/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `opin pairing list matrix`
  - `opin pairing approve matrix <CODE>`
...
- On startup, Opin resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
