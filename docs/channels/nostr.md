...
read_when:
  - You want Opin to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Opin to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`opin onboard`) and `opin channels add` list optional channel plugins.
...
```bash
opin plugins install @opin/nostr
```

Use a local checkout (dev workflows):

```bash
opin plugins install --link <path-to-opin>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "opin@example.com",
        "lud16": "opin@example.com"
      }
    }
  }
}
