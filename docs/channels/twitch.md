...
read_when:
  - Setting up Twitch chat integration for Opin
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Opin connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
opin plugins install @opin/twitch
```

Local checkout (when running from a git repo):

```bash
opin plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPIN_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "opin", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPIN_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPIN_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
