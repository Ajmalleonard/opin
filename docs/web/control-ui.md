...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/opin`)
...
If the page fails to load, start the Gateway first: `opin gateway`.
...
you revoke it with `opin devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.opin/opin.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Opin
...
Opin **blocks** Control UI connections without device identity.
