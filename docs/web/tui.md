...
```bash
opin tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `opin tui --deliver`
...
- Check the Gateway logs: `opin logs --follow`.
- Confirm the agent can run: `opin status` and `opin models status`.
...
- No agents in picker: check `opin agents list` and your routing config.
