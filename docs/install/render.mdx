...
Deploy Opin on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/opin/opin)
...
repository configures everything needed to run Opin:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPIN_STATE_DIR
        value: /data
      - key: OPIN_WORKSPACE_DIR
        value: /data/workspace
      - key: OPIN_GATEWAY_TOKEN
...
Legacy `OPIN_*` env aliases still work during the transition, but the Blueprint now uses `OPIN_*` as the canonical names.
...
The web dashboard is available at `https://<your-service>.onrender.com/opin`.
...
If you use the original Opin repository, Render will not auto-deploy your Opin. To update it, run a manual Blueprint sync from the dashboard.
...
For Opin, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Opin host.
