...
Deploy Opin on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/opinbot-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/opin` — Control UI

## What you get

- Hosted Opin Gateway + Control UI
...
- `OPIN_STATE_DIR=/data/.opin` (recommended)
- `OPIN_WORKSPACE_DIR=/data/workspace` (recommended)
- `OPIN_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Opin state + workspace so you can migrate to another host without losing config or memory.
