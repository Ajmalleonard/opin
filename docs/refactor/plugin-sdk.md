...
- Publish as `opin/plugin-sdk` (or export from core under `opin/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpinPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpinConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpinConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpinConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpinConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpinConfig): string;
  };
};
```
...
- Introduce `opin/plugin-sdk`.
- Add `api.runtime` to `OpinPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `opinRuntime: ">=2026.2.0"`).
