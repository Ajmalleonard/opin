...
Opin features that can generate provider usage or paid API calls.
...
- `opin status --usage` and `opin channels list` show provider **usage windows**
...
Opin can pick up credentials from:
...
`opin models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
