...
Opin integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Opin owns lifecycle when `channels.signal.autoStart=true`.
...
- Opin spawns `imsg rpc` as a child process (legacy iMessage integration).
