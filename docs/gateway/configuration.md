---
summary: "All configuration options for ~/.opin/opin.json with examples"
read_when:
...
Opin reads an optional **JSON5** config from `~/.opin/opin.json` (comments + trailing commas allowed).

If the file is missing, Opin uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.opin/workspace`). You usually only need a config to:
...
Opin only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `opin doctor`, `opin logs`, `opin health`, `opin status`, `opin service`, `opin help`).
- Run `opin doctor` to see the exact issues.
- Run `opin doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `opin config set`. Keep a backup of `~/.opin/opin.json`.
...
```bash
opin gateway call config.get --params '{}' # capture payload.hash
opin gateway call config.apply --params '{
  "raw": "{\\n  agents: { defaults: { workspace: \\"~/.opin/workspace\\" } }\\n}\\n",
  "baseHash": "<hash-from-config.get>",
  "sessionKey": "agent:main:whatsapp:dm:+25512345678",
  "restartDelayMs": 1000
}'
```
...
```bash
opin gateway call config.get --params '{}' # capture payload.hash
opin gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.opin/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@opin", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.opin/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.opin/workspace" }],
}
```
...
{ "$include": "/etc/opin/base.json5" } // absolute
...
```json5
// ~/.opin/clients/mueller/agents.json5
[
...
```json5
// ~/.opin/clients/mueller/broadcast.json5
{
...
Opin reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Opin runs your login shell and imports only the missing expected keys (never overrides).
...
Opin stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.opin/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.opin/credentials/oauth.json` (or `$OPIN_STATE_DIR/credentials/oauth.json`)
...
- `~/.opin/agent/*` (migrated by `opin doctor` into `~/.opin/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `OPIN_OAUTH_DIR`
- Agent dir (default agent root override): `OPIN_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Opin imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Opin derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/opin/opin-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/opin/opin.log`.
...
```json5
{
  channels: {
    whatsapp: {
      ...
      allowFrom: ["+25512345678", "+447700900123"],
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.opin/credentials/whatsapp/biz
          // authDir: "~/.opin/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `opin doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@opin", "opin"] } }],
  },
}
```
...
```json5
{
  agents: {
    list: [
      { id: "work", groupChat: { mentionPatterns: ["@workbot", "\\+25512345678"] } },
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@opin"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.opin/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.opin/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.opin/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.opin/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.opin/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.opin/workspace-home" },
      { id: "work", workspace: "~/.opin/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `opin.json`).
...
Opin starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["opin-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-opin",
...
Opin starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `opin plugins install @opin/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Opin starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Opin spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  channels: {
    imessage: {
      ...
      dmPolicy: "pairing", // pairing | allowlist | open | disabled
      allowFrom: ["+25512345678", "user@example.com", "chat_id:123"],
      historyLimit: 50, // include last N group messages as context (0 disables)
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Opin
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/opin" } },
}
```
...
When a file exceeds this limit, Opin logs a warning and injects a truncated
...
message envelopes). If unset, Opin uses the host timezone at runtime.
...
`[opin]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[opin]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[opin]"`, Opin will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Opin generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.opin/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"Opin"`).
...
Opin also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Opin currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `opin configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:opin`: all built-in Opin tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.opin/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.opin/sandboxes",
        docker: {
          image: "opin-sandbox:bookworm-slim",
          containerPrefix: "opin-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          enabled: false,
          image: "opin-sandbox-browser:bookworm-slim",
          containerPrefix: "opin-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Opin uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.opin/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Opin config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Opin writes/merges a `models.json` into
`~/.opin/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Opin uses
...
- Shortcut: `opin onboard --auth-choice opencode-zen`.
...
Shortcut: `opin onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `opin onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `opin onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `opin onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `opin onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `opin onboard --auth-choice minimax-api`.
...
- Override the agent config root with `OPIN_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.opin/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.opin/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.opin/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `direct`, `group`, and `thread`. Legacy `dm` key is accepted as an alias for `direct`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Opin stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.opin/skills` (workspace skills
...
from `~/.opin/extensions`, `<workspace>/.opin/extensions`, plus any
...
### `browser` (opin-managed browser)

Opin can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for opin and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Opin.app menubar, or `opin gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      opin: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/opin" }
...
- When a connection comes from one of these IPs, Opin uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `opin gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `OPIN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `OPIN_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Opin
...
- Opin.app watches `~/.opin/opin.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.opin/opin.json` (or `OPIN_CONFIG_PATH`) and applies changes automatically.
...
- `~/.opin/opin.json` (or `OPIN_CONFIG_PATH`)
...
- `OPIN_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `opin --dev …` → uses `~/.opin-dev` + shifts ports from base `19001`
- `opin --profile <name> …` → uses `~/.opin-<name>` (port via config/env/flags)
...
```bash
OPIN_CONFIG_PATH=~/.opin/a.json \
OPIN_STATE_DIR=~/.opin-a \
opin gateway --port 19001
```
...
Gmail helper config (used by `opin webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "opin@gmail.com",
...
- Set `OPIN_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Opin defaults `serve.path` to `/` so
...
Default root: `~/.opin/workspace/canvas`  
Default port: `18793` (chosen to avoid the opin browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__opin__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__opin__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.opin/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.opin/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.opin/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_opin-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `opin` (advertises `opin.local`). Override with `OPIN_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_opin-gw._tcp` under `~/.opin/dns/` using the configured discovery domain (example: `opin.internal.`).
