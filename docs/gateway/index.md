...
- Replaces the legacy `gateway` command. CLI entry point: `opin gateway`.
...
- Config hot reload watches `~/.opin/opin.json` (or `OPIN_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__opin__/canvas/` from `~/.opin/workspace/canvas`. Disable with `canvasHost.enabled=false` or `OPIN_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `OPIN_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `OPIN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.opin.*` may still exist)
- Linux: `opin-gateway-<profile>.service`
- Windows: `Opin Gateway (<profile>)`

Install metadata is embedded in the service config:

- `OPIN_SERVICE_MARKER=opin`
- `OPIN_SERVICE_KIND=gateway`
- `OPIN_SERVICE_VERSION=<version>`
...
```bash
opin --dev setup
opin --dev gateway --allow-unconfigured
...
- `OPIN_STATE_DIR=~/.opin-dev`
- `OPIN_CONFIG_PATH=~/.opin-dev/opin.json`
- `OPIN_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.opin/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `OPIN_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `OPIN_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `OPIN_CONFIG_PATH`
- unique `OPIN_STATE_DIR`
...
```bash
opin --profile main gateway install
opin --profile rescue gateway install
```

Example:

```bash
OPIN_CONFIG_PATH=~/.opin/a.json OPIN_STATE_DIR=~/.opin-a opin gateway --port 19001
OPIN_CONFIG_PATH=~/.opin/b.json OPIN_STATE_DIR=~/.opin-b opin gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `opin health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `opin`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `opin gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.opin.*` is cleaned up).
  - `opin doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Opin profile services.
  ...
  - Cleanup: `opin gateway uninstall` (current service) and `opin doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `opin gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Opin.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.opin.*` labels still unload cleanly).
- To stop it cleanly, use `opin gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `opin gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `opin gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

Opin installs a **systemd user service** by default on Linux/WSL2. We
...
`opin gateway install` writes the user unit. `opin doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/opin-gateway[-<profile>].service`:

```
[Unit]
Description=Opin Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/opin gateway --port 18789
Restart=always
RestartSec=5
Environment=OPIN_GATEWAY_TOKEN=
...
```
systemctl --user enable --now opin-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/opin-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now opin-gateway[-<profile>].service
```
...
- `opin gateway health|status` — request health/status over the Gateway WS.
- `opin message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `opin agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `opin gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `opin gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `opin gateway` and the legacy TCP control port.
