...
- During heartbeat runs, Opin treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `opin sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Opin skips the heartbeat run entirely (no model call).
...
```bash
opin system event --text "Check for urgent follow-ups" --mode now
```
