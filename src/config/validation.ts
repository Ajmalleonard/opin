...
import type { OpinConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpinConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: OpinConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpinSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpinConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpinConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as OpinConfig)),
    ),
  };
}
