...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-opin",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-opin#general");
  });
  ...
  it("derives session transcripts dir from OPIN_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPIN_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.OPIN_STATE_DIR = "/custom/state";
    try {
