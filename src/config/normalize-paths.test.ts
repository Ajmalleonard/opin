  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.opin/logs/opin.log" },
        hooks: {
          path: "~/.opin/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".opin", "logs", "opin.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".opin", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".opin", "telegram.token"),
      );
