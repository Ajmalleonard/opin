...
describe("legacy config detection", () => {
  ...
  it("migrates routing.allowFrom to channels.whatsapp.allowFrom when whatsapp configured", async () => {
    ...
    expect(res.config?.channels?.whatsapp?.allowFrom).toEqual(["+25512345678"]);
  ...
  it("migrates routing.groupChat.mentionPatterns to messages.groupChat.mentionPatterns", async () => {
    ...
    const { migrateLegacyConfig } = await import("./config.js");
    const res = migrateLegacyConfig({
      routing: { groupChat: { mentionPatterns: ["@opin"] } },
    });
    expect(res.changes).toContain(
      "Moved routing.groupChat.mentionPatterns → messages.groupChat.mentionPatterns.",
    );
    expect(res.config?.messages?.groupChat?.mentionPatterns).toEqual(["@opin"]);
  ...
  it('rejects whatsapp.dmPolicy="open" without allowFrom "*"', async () => {
    ...
    const res = validateConfigObject({
      channels: {
        whatsapp: { dmPolicy: "open", allowFrom: ["+25512345678"] },
      },
    });
    expect(res.ok).toBe(false);
    if (!res.ok) {
  ...
  it('rejects signal.dmPolicy="open" without allowFrom "*"', async () => {
    ...
    const { validateConfigObject } = await import("./config.js");
    const res = validateConfigObject({
      channels: { signal: { dmPolicy: "open", allowFrom: ["+25512345678"] } },
    });
    expect(res.ok).toBe(false);
    if (!res.ok) {
  ...
  it('rejects imessage.dmPolicy="open" without allowFrom "*"', async () => {
    ...
    const res = validateConfigObject({
      channels: {
        imessage: { dmPolicy: "open", allowFrom: ["+25512345678"] },
      },
    });
    expect(res.ok).toBe(false);
    if (!res.ok) {
