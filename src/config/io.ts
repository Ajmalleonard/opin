...
import type { OpinConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareOpinVersions } from "./version.js";
...
function coerceConfig(value: unknown): OpinConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpinConfig;
}
...
function stampConfigVersion(cfg: OpinConfig): OpinConfig {
...
function warnIfConfigFromFuture(cfg: OpinConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpinVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Opin (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: OpinConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpinConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as OpinConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpinConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpinConfig) {
    clearConfigCache();
...
// module scope. `OPIN_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPIN_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPIN_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): OpinConfig {
...
export async function writeConfigFile(cfg: OpinConfig): Promise<void> {
  clearConfigCache();
