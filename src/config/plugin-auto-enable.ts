import type { OpinConfig } from "./config.js";
...
function resolveChannelConfig(cfg: OpinConfig, channelId: string): Record<string, unknown> | null {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  const entry = channels?.[channelId];
  return isRecord(entry) ? entry : null;
}

function isTelegramConfigured(cfg: OpinConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: OpinConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: OpinConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: OpinConfig): boolean {
...
function isIMessageConfigured(cfg: OpinConfig): boolean {
...
function isWhatsAppConfigured(cfg: OpinConfig): boolean {
...
function isGenericChannelConfigured(cfg: OpinConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: OpinConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OpinConfig, providerId: string): boolean {
...
function resolveConfiguredPlugins(cfg: OpinConfig, env: NodeJS.ProcessEnv): PluginEnableChange[] {
...
function isPluginExplicitlyDisabled(cfg: OpinConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: OpinConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: OpinConfig, pluginId: string): OpinConfig {
...
function registerPluginEntry(cfg: OpinConfig, pluginId: string): OpinConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
