...
export function parseOpinVersion(raw: string | null | undefined): OpinVersion | null {
  if (!raw) {
    return null;
  }
...
export function compareOpinVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseOpinVersion(a);
  const parsedB = parseOpinVersion(b);
  if (!parsedA || !parsedB) {
    return null;
  }
