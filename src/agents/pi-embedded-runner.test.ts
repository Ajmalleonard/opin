...
vi.mock("./pi-embedded-runner/run/attempt.js", async () => {
  ...
  return {
    ...actual,
    runEmbeddedAttempt: vi.fn((...args: Parameters<typeof actual.runEmbeddedAttempt>) =>
      actual.runEmbeddedAttempt(...args),
    ),
  };
});
...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opin-embedded-agent-"));
...
const makeOpenAiConfig = (modelIds: string[]) =>
  ...
  }) satisfies OpinConfig;

const ensureModels = (cfg: OpinConfig) => ensureOpinModelsJson(cfg, agentDir) as unknown;
...
describe("runEmbeddedPiAgent", () => {
  ...
  it("overrides reasoningLevel to off when messageChannel is an external channel like whatsapp", async () => {
    const sessionFile = nextSessionFile();
    const cfg = makeOpenAiConfig(["mock-1"]);
    await ensureModels(cfg);

    const { runEmbeddedAttempt } = await import("./pi-embedded-runner/run/attempt.js");
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mockAttempt = runEmbeddedAttempt as any;
    ...
    const lastCallParams = mockAttempt.mock.calls[0][0];
    expect(lastCallParams.reasoningLevel).toBe("off");
    ...
    expect(mockAttempt).toHaveBeenCalled();
    const webchatCallParams = mockAttempt.mock.calls[0][0];
    expect(webchatCallParams.reasoningLevel).toBe("on");
  });
});
