...
const defaultTools = createOpinCodingTools();

describe("createOpinCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opin-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from opin read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "opin-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "opin-workspace"),
    ...
    const tools = createOpinCodingTools({ sandbox });
