...
/**
 * Cleanup suspended Opin CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: OpinConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opin-cli-images-"));
  const paths: string[] = [];
