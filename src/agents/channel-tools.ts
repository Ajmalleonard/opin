...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as OpinConfig);
...
export function listChannelAgentTools(params: { cfg?: OpinConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: OpinConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {
