...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "opin.sandbox=1");
  args.push("--label", `opin.sessionKey=${params.scopeKey}`);
  args.push("--label", `opin.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `opin.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("opin.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`opin sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`opin sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("opin sandbox recreate --all");
}
