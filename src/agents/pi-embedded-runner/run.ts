...
export async function runEmbeddedPiAgent(
  ...
  const isInternal = !channelHint || isInternalMessageChannel(channelHint);
  if (!isInternal && params.reasoningLevel !== "off") {
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      const agentDir = params.agentDir ?? resolveOpinAgentDir();
      const fallbackConfigured =
        (params.config?.agents?.defaults?.model?.fallbacks?.length ?? 0) > 0;
      await ensureOpinModelsJson(params.config, agentDir);
      ...
      try {
        while (true) {
          ...
          if (!isProbeSession && params.sessionFile && model && authStorage) {
            const { extractAndSavePreferences } =
              await import("../../memory/preference-profile.js");
            void extractAndSavePreferences({
              sessionFile: params.sessionFile,
              agentId: workspaceResolution.agentId,
              model,
              authStorage,
