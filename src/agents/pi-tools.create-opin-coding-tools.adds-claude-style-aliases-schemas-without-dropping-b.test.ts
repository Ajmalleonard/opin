...
describe("createOpinCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createOpinCodingTools({
      config,
    ...
    const anthropicTools = createOpinCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createOpinCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createOpinCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
