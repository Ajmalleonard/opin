...
type WebSearchConfig = NonNullable<OpinConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OpinConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("opin configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.opin.squareexp.com/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://opin.squareexp.com",
      "X-Title": "Opin Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
