...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "opin-models-" });
}

const MODELS_CONFIG: OpinConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureOpinModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureOpinModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureOpinModelsJson } = await import("./models-config.js");
      const { resolveOpinAgentDir } = await import("./agent-paths.js");

      await ensureOpinModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveOpinAgentDir(), "models.json");
