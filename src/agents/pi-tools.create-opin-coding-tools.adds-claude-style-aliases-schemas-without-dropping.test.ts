...
import { __testing, createOpinCodingTools } from "./pi-tools.js";
...
describe("createOpinCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOpinCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opin-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "opin-outside.txt");
