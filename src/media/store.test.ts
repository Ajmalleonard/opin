...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "OPIN_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "opin-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.OPIN_STATE_DIR = path.join(home, ".opin");
    ...
    await fs.mkdir(path.join(home, ".opin"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.opin${path.sep}media`);
