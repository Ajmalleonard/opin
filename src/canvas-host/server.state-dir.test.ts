...
describe("canvas host state dir defaults", () => {
  ...
  it("uses OPIN_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opin-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Opin Canvas");
    } finally {
      await handler.close();
