...
import type { OpinConfig, ConfigFileSnapshot } from "../config/config.js";
...
import {
  resolveProjectScopedValue,
  type ProjectScopedManifestKey,
...
function summarizeGroupPolicy(cfg: OpinConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: OpinConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep OPIN_STATE_DIR on a local-only volume and re-run "${formatCliCommand("opin security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: OpinConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation: "Prefer OPIN_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: OpinConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: OpinConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: OpinConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: OpinConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: OpinConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: OpinConfig): boolean {
...
function isBrowserEnabled(cfg: OpinConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: OpinConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectExposureMatrixFindings(cfg: OpinConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
async function readPluginManifestExtensions(pluginPath: string): Promise<string[]> {
  ...
  const parsed = JSON.parse(raw) as
    | Partial<Record<ProjectScopedManifestKey, { extensions?: unknown }>>
    | Record<string, unknown>
    | null;
  const extensions = resolveProjectScopedValue<{ extensions?: unknown }>(parsed)?.extensions;
...
function listWorkspaceDirs(cfg: OpinConfig): string[] {
...
export async function collectPluginsCodeSafetyFindings(params: {
  ...
  const entries = await fs.readdir(extensionsDir, { withFileTypes: true }).catch((err) => {
    findings.push({
      ...
      remediation:
        "Check file permissions and plugin layout, then rerun `opin security audit --deep`.",
    });
    return [];
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all opin.extensions entries stay inside the plugin directory.",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your Opin extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (entry.skill.source === "opin-bundled") {
        continue;
      }
      ...
      const summary = await scanDirectoryWithSummary(skillDir).catch((err) => {
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `opin security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
