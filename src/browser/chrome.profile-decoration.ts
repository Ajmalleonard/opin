...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".opin-profile-decorated");
}
 ...
 */
export function decorateOpinProfile(userDataDir: string, opts?: { name?: string; color?: string }) {
  const desiredName = opts?.name ?? DEFAULT_OPIN_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPIN_BROWSER_COLOR).toUpperCase();
