import type { BrowserConfig, BrowserProfileConfig, OpinConfig } from "../config/config.js";
...
import {
  DEFAULT_OPIN_BROWSER_COLOR,
  DEFAULT_OPIN_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_OPIN_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OPIN_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "opin" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPIN_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPIN_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Opin browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for opin/opin CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpinConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPIN_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "opin";
