...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "opin-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "opin-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "opin-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "opin-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.opin when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "opin-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPIN_HOME over HOME", () => {
    vi.stubEnv("OPIN_HOME", "/srv/opin-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/opin-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPIN_HOME prefix when OPIN_HOME is set", () => {
    vi.stubEnv("OPIN_HOME", "/srv/opin-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/opin-home")}/.opin/opin.json`)).toBe(
      "$OPIN_HOME/.opin/opin.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPIN_HOME replacement when OPIN_HOME is set", () => {
    vi.stubEnv("OPIN_HOME", "/srv/opin-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomeInString(`config: ${path.resolve("/srv/opin-home")}/.opin/opin.json`)).toBe(
      "config: $OPIN_HOME/.opin/opin.json",
    );
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/opin")).toBe(path.resolve(os.homedir(), "opin"));
  });
  ...
  it("prefers OPIN_HOME for tilde expansion", () => {
    vi.stubEnv("OPIN_HOME", "/srv/opin-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/opin")).toBe(path.resolve("/srv/opin-home", "opin"));
