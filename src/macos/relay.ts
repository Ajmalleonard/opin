...
declare const __OPIN_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __OPIN_VERSION__ === "string" && __OPIN_VERSION__) ||
  process.env.OPIN_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensureOpinCliOnPath } = await import("../infra/path-env.js");
  ensureOpinCliOnPath();
  ...
  process.on("uncaughtException", (error) => {
    console.error("[opin] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[opin] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
