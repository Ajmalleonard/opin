...
import { ensureOpinCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.OPIN_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed = process.env.OPIN_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureOpinCliOnPath({ pathEnv: process.env.PATH ?? "" });
