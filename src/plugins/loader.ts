...
import { discoverOpinPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OpinPluginDefinition;
  register?: OpinPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OpinPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OpinPluginDefinition;
...
export function loadOpinPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOpinPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "opin/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OpinPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as OpinPluginModule;
    } catch (err) {
