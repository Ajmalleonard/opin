...
import { loadOpinPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.OPIN_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `opin-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, params.manifestFilename ?? "opin.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.OPIN_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadOpinPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.OPIN_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadOpinPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOpinPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it.each(["opin/plugin-sdk"])("loads plugins that import %s", (sdkSpecifier) => {
    process.env.OPIN_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
    const plugin = writePlugin({
      id: "sdk-opin",
      body: `import { emptyPluginConfigSchema } from "${sdkSpecifier}";
export default {
  id: "sdk-opin",
