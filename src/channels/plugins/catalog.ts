...
import type { OpinPackageManifest } from "../../plugins/manifest.js";
...
import { discoverOpinPlugins } from "../../plugins/discovery.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, OpinPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["OPIN_PLUGIN_CATALOG_PATHS", "OPIN_MPM_CATALOG_PATHS"];

type ManifestKey = ProjectScopedManifestKey;
...
function toChannelMeta(params: {
  channel: NonNullable<OpinPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: OpinPackageManifest;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = resolveProjectScopedValue<OpinPackageManifest>(entry);
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverOpinPlugins({ workspaceDir: options.workspaceDir });
