...
function setSignalDmPolicy(cfg: OpinConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function setSignalAllowFrom(cfg: OpinConfig, accountId: string, allowFrom: string[]): OpinConfig {
...
async function promptSignalAllowFrom(params: {
  ...
  const entry = await params.prompter.text({
    ...
    placeholder: "+25512345678, uuid:123e4567-e89b-12d3-a456-426614174000",
...
export const signalOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "Opin"',
        "Scan QR in Signal → Linked Devices",
        `Then run: ${formatCliCommand("opin gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
