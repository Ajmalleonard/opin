...
function setSlackDmPolicy(cfg: OpinConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Opin";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Opin`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: OpinConfig, allowFrom: string[]): OpinConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
