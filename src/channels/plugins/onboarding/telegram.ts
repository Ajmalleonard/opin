...
function setTelegramDmPolicy(cfg: OpinConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
async function noteTelegramTokenHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Docs home: https://docs.opin.squareexp.com",
    ].join("\n"),
    "Telegram bot token",
  );
}
...
async function noteTelegramUserIdHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      `1) DM your bot, then read from.id in \`${formatCliCommand("opin logs --follow")}\` (safest)`,
