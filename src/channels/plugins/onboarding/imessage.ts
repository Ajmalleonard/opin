...
function setIMessageDmPolicy(cfg: OpinConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function setIMessageAllowFrom(cfg: OpinConfig, accountId: string, allowFrom: string[]): OpinConfig {
...
async function promptIMessageAllowFrom(params: {
  ...
  const entry = await params.prompter.text({
    ...
    placeholder: "+25512345678, user@example.com, chat_id:123",
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds }) => {
    ...
    await prompter.note(
      [
        ...
        "Ensure Opin has Full Disk Access to Messages DB.",
