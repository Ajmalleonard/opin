...
function setWhatsAppDmPolicy(cfg: OpinConfig, dmPolicy: DmPolicy): OpinConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: OpinConfig, allowFrom?: string[]): OpinConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: OpinConfig, selfChatMode: boolean): OpinConfig {
...
async function detectWhatsAppLinked(cfg: OpinConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Opin can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Opin" },
    ],
  });
  ...
  if (mode === "keep") {
    ...
    const allowRaw = await prompter.text({
      ...
      placeholder: "+25512345678, +447700900123",
      validate: (value) => {
        const raw = String(value ?? "").trim();
        if (!raw) {
          return "Required";
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("opin channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
