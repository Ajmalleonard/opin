...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-opin-message-channel") ?? "");
  const accountId = getHeader(req, "x-opin-account-id")?.trim() || undefined;
  ...
  const allTools = createOpinTools({
