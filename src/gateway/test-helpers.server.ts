...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.OPIN_STATE_DIR;
  previousConfigPath = process.env.OPIN_CONFIG_PATH;
  previousSkipBrowserControl = process.env.OPIN_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.OPIN_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.OPIN_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.OPIN_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opin-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPIN_STATE_DIR = path.join(tempHome, ".opin");
  delete process.env.OPIN_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.OPIN_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OPIN_SKIP_GMAIL_WATCHER = "1";
  process.env.OPIN_SKIP_CANVAS_HOST = "1";
  process.env.OPIN_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "opin-test-no-bundled-extensions")
    : "opin-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "opin-test-"))
    : path.join(tempHome, ".opin-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.OPIN_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.OPIN_CONFIG_PATH;
    } else {
      process.env.OPIN_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.OPIN_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.OPIN_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.OPIN_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPIN_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.OPIN_SKIP_CANVAS_HOST;
    } else {
      process.env.OPIN_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.OPIN_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.OPIN_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.OPIN_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPIN_GATEWAY_TOKEN = fallbackToken;
  }
