...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.OPIN_SKIP_CHANNELS;
    prevSkipGmail = process.env.OPIN_SKIP_GMAIL_WATCHER;
    process.env.OPIN_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.OPIN_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.OPIN_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPIN_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
