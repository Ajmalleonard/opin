...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.OPIN_CONFIG_PATH;
  const prevDisableCache = process.env.OPIN_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "opin-canvas-auth-test-"));
  const configPath = path.join(dir, "opin.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.OPIN_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.OPIN_DISABLE_CONFIG_CACHE;
    } else {
      process.env.OPIN_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
