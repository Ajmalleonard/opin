...
homepage: https://docs.opin.squareexp.com/hooks#command-logger
metadata:
  {
    "opin":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.opin/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.opin/logs/commands.log ~/.opin/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/opin`:
   ```
   /home/username/.opin/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
