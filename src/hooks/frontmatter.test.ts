...
`;
    ...
    expect(parsed.opin.emoji).toBe("💾");
    expect(parsed.opin.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.opin.emoji).toBe("📝");
    expect(parsed.opin.events).toEqual(["command"]);
    expect(parsed.opin.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.opin.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"opin": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"opin": {"events": ["test"]}}');
  });
...
describe("resolveOpinMetadata", () => {
  it("extracts opin metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpinMetadata(frontmatter);
  ...
  it("returns undefined when opin key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        opin: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Opin" },
            { id: "npm", kind: "npm", package: "@opin/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@opin/hook");
  });
...
homepage: https://docs.opin.squareexp.com/hooks#session-memory
metadata:
  {
    "opin":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Opin" }],
      },
  }
---
...
`;
    ...
    const opin = resolveOpinMetadata(frontmatter);
    expect(opin).toBeDefined();
    expect(opin?.emoji).toBe("💾");
    expect(opin?.events).toEqual(["command:new"]);
    expect(opin?.requires?.config).toEqual(["workspace.dir"]);
    expect(opin?.install?.[0].kind).toBe("bundled");
  });
