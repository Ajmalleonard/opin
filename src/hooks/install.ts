...
} & Partial<Record<ProjectScopedManifestKey, { hooks?: string[] }>>;
...
async function ensureOpinHooks(manifest: HookPackageManifest) {
  const hooks = resolveProjectScopedValue<{ hooks?: string[] }>(manifest)?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing opin.hooks (legacy opin.hooks also supported)");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json opin.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(params: {
  ...
  try {
    hookEntries = await ensureOpinHooks(manifest);
  } catch (err) {
...
export async function installHooksFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opin-hook-"));
...
export async function installHooksFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opin-hook-pack-"));
