...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPIN_GATEWAY_PORT).toBe("18789");
    expect(env.OPIN_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPIN_SERVICE_MARKER).toBe("opin");
    expect(env.OPIN_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPIN_SERVICE_VERSION).toBe("string");
    expect(env.OPIN_SYSTEMD_UNIT).toBe("opin-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPIN_LAUNCHD_LABEL).toBe("ai.opin.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPIN_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPIN_SYSTEMD_UNIT).toBe("opin-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPIN_LAUNCHD_LABEL).toBe("ai.opin.work");
    }
  });
});
