...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.OPIN_STATE_DIR;
  const configPath = env.OPIN_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    OPIN_PROFILE: profile,
    OPIN_STATE_DIR: stateDir,
    OPIN_CONFIG_PATH: configPath,
    OPIN_GATEWAY_PORT: String(port),
    OPIN_GATEWAY_TOKEN: token,
    OPIN_LAUNCHD_LABEL: resolvedLaunchdLabel,
    OPIN_SYSTEMD_UNIT: systemdUnit,
    OPIN_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    OPIN_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    OPIN_STATE_DIR: stateDir,
    OPIN_CONFIG_PATH: configPath,
    OPIN_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    OPIN_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    OPIN_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    OPIN_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    OPIN_LOG_PREFIX: "node",
    OPIN_SERVICE_MARKER: NODE_SERVICE_MARKER,
    OPIN_SERVICE_KIND: NODE_SERVICE_KIND,
    OPIN_SERVICE_VERSION: VERSION,
  };
}
