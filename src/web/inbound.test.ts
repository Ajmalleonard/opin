...
describe("web inbound helpers", () => {
    ...
    const body = extractText({
      contactMessage: {
        ...
        vcard: [
          ...
          "VERSION:3.0",
          "FN:Ada Lovelace",
          "TEL;TYPE=CELL:+25512345678",
          "END:VCARD",
        ].join("\n"),
      },
    } as unknown as import("@whiskeysockets/baileys").proto.IMessage);
    expect(body).toBe("<contact: Ada Lovelace, +25512345678>");
  });
  ...
  it("trims and skips empty WhatsApp vcard phones", () => {
    ...
    expect(body).toBe("<contact: Ada Lovelace, +25512345678 (+1 more)>");
  });
