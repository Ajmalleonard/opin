...
import type { OpinConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpinConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("opin security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("opin configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("opin config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("opin doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("opin config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
