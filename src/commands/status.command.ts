...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Opin status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("opin security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("opin security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.opin.squareexp.com/faq");
  runtime.log("Troubleshooting: https://docs.opin.squareexp.com/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("opin status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("opin logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("opin status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("opin gateway probe")}`);
  }
}
