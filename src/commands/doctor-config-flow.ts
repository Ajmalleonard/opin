...
function stripUnknownConfigKeys(config: OpinConfig): {
  config: OpinConfig;
  removed: string[];
} {
  const parsed = OpinSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: OpinConfig) {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".opin");
  const targetPath = path.join(targetDir, "opin.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".opin", "opin.json"),
    path.join(home, ".opin", "opin.json"),
    path.join(home, ".opinbot", "opinbot.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("opin doctor --fix")}" to apply legacy migrations.`);
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("opin doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "opin doctor --fix" to remove these keys.');
    }
  }
