...
export function listAgentEntries(cfg: OpinConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OpinConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpinConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpinConfig): AgentSummary[] {
