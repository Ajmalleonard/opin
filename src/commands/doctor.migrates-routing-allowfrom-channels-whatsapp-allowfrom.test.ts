...
describe("doctor command", () => {
  it("migrates routing.allowFrom to channels.whatsapp.allowFrom", { timeout: 60_000 }, async () => {
    readConfigFileSnapshot.mockResolvedValue({
      ...
      exists: true,
      raw: "{}",
      parsed: { routing: { allowFrom: ["+25512345678"] } },
      valid: false,
      config: {},
      issues: [
        {
    ...
    migrateLegacyConfig.mockReturnValue({
      config: { channels: { whatsapp: { allowFrom: ["+25512345678"] } } },
      changes: ["Moved routing.allowFrom → channels.whatsapp.allowFrom."],
    });
  ...
  it("skips legacy gateway services migration", { timeout: 60_000 }, async () => {
    ...
    findLegacyGatewayServices.mockResolvedValueOnce([
      {
        platform: "darwin",
        label: "com.steipete.opin.gateway",
        detail: "loaded",
      },
    ]);
  ...
  it("offers to update first for git checkouts", async () => {
    ...
    const root = "/tmp/opin";
    resolveOpinPackageRoot.mockResolvedValueOnce(root);
