...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.OPIN_GATEWAY_PASSWORD,
    disableConfigCache: process.env.OPIN_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("OPIN_STATE_DIR", prev.stateDir);
  restoreEnvVar("OPIN_CONFIG_PATH", prev.configPath);
  restoreEnvVar("OPIN_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("OPIN_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("OPIN_SKIP_CRON", prev.skipCron);
  restoreEnvVar("OPIN_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("OPIN_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("OPIN_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("OPIN_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.OPIN_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "opin.json");
  process.env.HOME = tempHome;
  process.env.OPIN_STATE_DIR = tempHome;
  process.env.OPIN_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("opin-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("opin-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("opin-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("opin-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("opin-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("opin-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
