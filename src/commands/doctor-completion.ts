...
async function resolveCliEntryPath(root: string): Promise<string | null> {
  for (const candidate of ["opin.mjs", "opin.mjs"]) {
    ...
    try {
      ...
      // Try the next entrypoint candidate.
    }
  }
  return null;
}
...
async function generateCompletionCache(): Promise<boolean> {
  const root = await resolveOpinPackageRoot({
...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(opin completion ...)` */
  usesSlowPattern: boolean;
};

/** Check the status of shell completion for the current shell. */
export async function checkShellCompletionStatus(binName = "opin"): Promise<ShellCompletionStatus> {
 ...
 */
export async function ensureCompletionCacheExists(binName = "opin"): Promise<boolean> {
  const shell = resolveShellFromEnv() as CompletionShell;
  const cacheExists = await completionCacheExists(shell, binName);
