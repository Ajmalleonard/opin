...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opin-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.OPIN_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "opin");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.OPIN_STATE_DIR = stateDir;
    process.env.OPIN_CONFIG_PATH = path.join(stateDir, "opin.json");
