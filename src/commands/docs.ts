...
const SEARCH_TOOL = "https://docs.opin.squareexp.com/mcp.SearchOpin";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.opin.squareexp.com");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('opin docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.opin.squareexp.com/");
      runtime.log(`Search: ${formatCliCommand('opin docs "your query"')}`);
    }
    return;
  }
