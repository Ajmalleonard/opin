...
const requireAgentDir = () => {
  const agentDir = process.env.OPIN_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPIN_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.OPIN_STATE_DIR;
  const previousAgentDir = process.env.OPIN_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.OPIN_AGENT_DIR;
    } else {
      process.env.OPIN_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.OPIN_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OPIN_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opin-oauth-"));
    process.env.OPIN_STATE_DIR = tempStateDir;
    process.env.OPIN_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPIN_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPIN_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opin-minimax-"));
    process.env.OPIN_STATE_DIR = tempStateDir;
    process.env.OPIN_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPIN_AGENT_DIR;
...
describe("applyXiaomiConfig", () => {
  it("adds Xiaomi provider with correct settings", () => {
    ...
    expect(cfg.agents?.defaults?.model?.primary).toBe("xiaomi/mimo-v2.5");
  });
