...
beforeEach(() => {
  ...
  resolveOpinPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPIN_STATE_DIR;
  originalUpdateInProgress = process.env.OPIN_UPDATE_IN_PROGRESS;
  process.env.OPIN_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "opin-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPIN_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPIN_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPIN_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveOpinPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadOpinPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/opin-root.js", () => ({
  resolveOpinPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "opin-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.OPIN_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OPIN_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/opin.json",
      exists: true,
      raw: "{}",
    ...
    process.env.OPIN_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
