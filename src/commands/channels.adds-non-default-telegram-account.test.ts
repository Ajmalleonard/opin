...
describe("channels command", () => {
  ...
  it("adds a second signal account with a distinct name", async () => {
    ...
    expect(next.channels?.signal?.accounts?.lab?.account).toBe("+25512345678");
  ...
  it("surfaces Discord privileged intent issues in channels status output", () => {
    ...
    expect(lines.join("\n")).toMatch(/Run: (?:opin|opin)( --profile isolated)? doctor/);
  });
        ...
        telegram: [
          {
            ...
            enabled: true,
            configured: true,
            probe: { ok: true, bot: { username: "opin_bot" } },
          },
        ],
      },
    });
    expect(lines.join("\n")).toMatch(/bot:@opin_bot/);
  });
