...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPIN_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPIN_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPIN_GATEWAY_TOKEN");
    expect(message).not.toContain("OPIN_GATEWAY_PASSWORD");
  });
