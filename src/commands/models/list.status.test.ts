...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpinAgentDir: vi.fn().mockReturnValue("/tmp/opin-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/opin-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/opin-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpinAgentDir: mocks.resolveOpinAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpinAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/opin-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/opin-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/opin-agent-custom");
