...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpinAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("opin models auth setup-token")}\` or \`${formatCliCommand("opin configure")}\`.`
          : `Run \`${formatCliCommand("opin configure")}\` or set an API key env var.`;
