...
import { type OpinConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function updateConfig(mutator: (cfg: OpinConfig) => OpinConfig): Promise<OpinConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: OpinConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpinConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("opin agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
