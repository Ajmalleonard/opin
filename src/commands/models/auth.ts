...
import { readConfigFileSnapshot, type OpinConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: OpinConfig, model: string): OpinConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("opin plugins install")}\`.`,
    );
  }
