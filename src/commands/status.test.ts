...
beforeAll(() => {
  previousProfile = process.env.OPIN_PROFILE;
  process.env.OPIN_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.OPIN_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/opin-root.js", () => ({
  resolveOpinPackageRoot: vi.fn().mockResolvedValue("/tmp/opin"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/opin/pnpm-lock.yaml",
      markerPath: "/tmp/opin/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Opin status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("opin status --all") ||
          l.includes("opin --profile isolated status --all") ||
          l.includes("opin status --all") ||
          l.includes("opin --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OPIN_GATEWAY_TOKEN = "abcd1234";
    try {
