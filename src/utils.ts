...
export function resolveConfigDir(
  ...
  const override = env.OPIN_STATE_DIR?.trim() || env.OPIN_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const home = resolveRequiredHomeDir(env, homedir);
  const newDir = path.join(home, ".opin");
  try {
  ...
  const legacyDir = path.join(home, ".opin");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPIN_HOME?.trim() || process.env.OPIN_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: process.env.OPIN_HOME?.trim() ? "$OPIN_HOME" : "$OPIN_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPIN_STATE_DIR env var.
