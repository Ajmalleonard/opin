...
import { isTruthyEnvValue, normalizeBrandEnv, normalizeZaiEnv } from "./env.js";

describe("normalizeBrandEnv", () => {
  it("maps OPIN_* variables into OPIN_* when missing", () => {
    const env = {
      OPIN_STATE_DIR: "/tmp/opin-state",
      OPIN_GATEWAY_TOKEN: "opin-token",
    } as NodeJS.ProcessEnv;

    normalizeBrandEnv(env);

    expect(env.OPIN_STATE_DIR).toBe("/tmp/opin-state");
    expect(env.OPIN_GATEWAY_TOKEN).toBe("opin-token");
  });

  it("does not override existing OPIN_* variables", () => {
    const env = {
      OPIN_STATE_DIR: "/tmp/opin-state",
    } as NodeJS.ProcessEnv;
