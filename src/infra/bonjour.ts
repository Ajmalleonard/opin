...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPIN_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Opin";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Opin\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.OPIN_MDNS_HOSTNAME?.trim() || process.env.OPIN_MDNS_HOSTNAME?.trim() || "opin";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
