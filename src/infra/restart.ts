...
export function triggerOpinRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(process.env.OPIN_SYSTEMD_UNIT, process.env.OPIN_PROFILE);
  ...
  const label =
    process.env.OPIN_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.OPIN_PROFILE);
