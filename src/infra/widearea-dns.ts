...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPIN_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*opin-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "opin";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "opin");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "opin-gw");
  ...
  records.push(`_opin-gw._tcp IN PTR ${instanceLabel}._opin-gw._tcp`);
  records.push(`${instanceLabel}._opin-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._opin-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; opin-content-hash: ${contentHash}\n${contentBody}`;
}
