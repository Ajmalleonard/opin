...
function candidateBinDirs(opts: EnsureOpinPathOpts): string[] {
  ...
  // Bundled macOS app: `opin` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "opin");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/opin` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "opin"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `opin` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpinCliOnPath(opts: EnsureOpinPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPIN_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.OPIN_PATH_BOOTSTRAPPED = "1";
