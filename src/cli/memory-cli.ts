...
export function registerMemoryCli(program: Command) {
  const memory = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.opin.squareexp.com/cli/memory")}\n`,
    );
  ...
  addGatewayClientOptions(
    memory
      .command("show")
      .description("Show the persistent preference profile for the agent")
      .option("--agent <id>", "Agent ID")
      .action(async (opts) => {
        try {
          const profile = (await callGatewayFromCli("memory.profile.get", opts, {
            agentId: opts.agent,
          })) as { preferences?: string; lastUpdated?: string };
          if (profile.preferences) {
            defaultRuntime.log(profile.preferences);
            ...
            defaultRuntime.log("No preference profile established yet.");
          }
        } catch (err) {
  ...
  addGatewayClientOptions(
    memory
      .command("edit")
      .description("Edit the persistent preference profile")
      .argument("[preferences]", "Direct preferences text to set")
      .option("--agent <id>", "Agent ID")
      .action(async (preferences, opts) => {
        try {
          if (typeof preferences === "string" && preferences.trim()) {
            await callGatewayFromCli("memory.profile.set", opts, {
              preferences,
              agentId: opts.agent,
            });
            defaultRuntime.log("Preferences updated.");
          } else {
            const current = (await callGatewayFromCli("memory.profile.get", opts, {
              agentId: opts.agent,
            })) as { preferences?: string; lastUpdated?: string };
            const initialText = current.preferences || "";

            const tempFile = path.join(os.tmpdir(), `opin-memory-${Date.now()}.md`);
            await fs.writeFile(tempFile, initialText, "utf8");

            const editor = process.env.EDITOR || "nano";
            const { spawnSync } = await import("node:child_process");
            spawnSync(editor, [tempFile], { stdio: "inherit" });

            const updatedText = await fs.readFile(tempFile, "utf8");
            try {
              await fs.unlink(tempFile);
            } catch {}

            if (updatedText.trim() === initialText.trim()) {
              defaultRuntime.log("No changes made.");
              return;
            }
            await callGatewayFromCli("memory.profile.set", opts, {
              preferences: updatedText,
              agentId: opts.agent,
            });
            defaultRuntime.log("Preferences updated.");
          }
  ...
  addGatewayClientOptions(
    memory
      .command("clear")
      .description("Clear the persistent preference profile")
      .option("--agent <id>", "Agent ID")
      .action(async (opts) => {
        try {
          await callGatewayFromCli("memory.profile.clear", opts, { agentId: opts.agent });
          defaultRuntime.log("Preferences cleared.");
        } catch (err) {
          defaultRuntime.error(danger(String(err)));
          defaultRuntime.exit(1);
        }
