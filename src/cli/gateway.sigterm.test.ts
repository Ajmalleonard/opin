...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "opin-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...
      OPIN_SKIP_GMAIL_WATCHER: "1",
      OPIN_SKIP_CRON: "1",
      OPIN_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPIN_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "opin-entry-bootstrap.mjs");
