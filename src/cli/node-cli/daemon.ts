...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("opin node install"), formatCliCommand("opin node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("opin node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    OPIN_LOG_PREFIX: baseEnv.OPIN_LOG_PREFIX ?? "node",
