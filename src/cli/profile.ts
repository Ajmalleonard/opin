...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.opin${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.OPIN_PROFILE = profile;
  env.OPIN_PROFILE = profile;

  const stateDir =
    env.OPIN_STATE_DIR?.trim() ||
    env.OPIN_STATE_DIR?.trim() ||
    resolveProfileStateDir(profile, env, homedir);
  if (!env.OPIN_STATE_DIR?.trim()) {
    env.OPIN_STATE_DIR = stateDir;
  }
  if (!env.OPIN_STATE_DIR?.trim()) {
    env.OPIN_STATE_DIR = stateDir;
  }
  ...
  const configPath =
    ...
    env.OPIN_CONFIG_PATH?.trim() ||
    path.join(stateDir, "opin.json");
  if (!env.OPIN_CONFIG_PATH?.trim()) {
    env.OPIN_CONFIG_PATH = configPath;
  }
  if (!env.OPIN_CONFIG_PATH?.trim()) {
    env.OPIN_CONFIG_PATH = configPath;
  }
  ...
  if (profile === "dev") {
    const gatewayPort = env.OPIN_GATEWAY_PORT?.trim() || "19001";
    if (!env.OPIN_GATEWAY_PORT?.trim()) {
      env.OPIN_GATEWAY_PORT = gatewayPort;
    }
    if (!env.OPIN_GATEWAY_PORT?.trim()) {
      env.OPIN_GATEWAY_PORT = gatewayPort;
    }
