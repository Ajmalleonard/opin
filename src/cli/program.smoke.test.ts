...
describe("cli program (smoke)", () => {
  ...
  it("registers ops command", () => {
    const program = buildProgram();
    const names = program.commands.map((command) => command.name());
    expect(names).toContain("ops");
  });

  it("runs ops action catalog without connector config", async () => {
    const program = buildProgram();
    await program.parseAsync(["ops", "actions", "list", "--role", "finance"], { from: "user" });
    expect(runtime.log).toHaveBeenCalledWith(expect.stringContaining("invoices.create"));
  });
