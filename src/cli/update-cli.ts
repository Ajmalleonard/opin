...
import { resolveOpinPackageRoot } from "../infra/opin-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "opin doctor entry": "Checking health entrypoint",
  "opin doctor": "Running health checks",
...
const DEFAULT_PACKAGE_NAME = "opincore";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "opin", "opin"]);
const CLI_NAME = resolveCliName();
const REPO_URL = process.env.OPIN_REPO_URL?.trim() || process.env.OPIN_REPO_URL?.trim() || null;
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("opincore@")) {
    return trimmed.slice("opincore@".length);
  }
  if (trimmed.startsWith("opin@")) {
    return trimmed.slice("opin@".length);
  }
  if (trimmed.startsWith("opin@")) {
    return trimmed.slice("opin@".length);
  }
...
async function resolveCliEntryPath(root: string): Promise<string | null> {
  for (const candidate of ["opin.mjs", "opin.mjs"]) {
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = await resolveCliEntryPath(root);
  if (!binPath) {
    return;
  }
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("opin completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.OPIN_GIT_DIR?.trim() || process.env.OPIN_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!REPO_URL) {
    throw new Error(
      "Set OPIN_REPO_URL to your Opin git repository before switching this install to the dev channel.",
    );
  }
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPIN_GIT_DIR points at a non-git directory: ${params.dir}. Set OPIN_GIT_DIR (or legacy OPIN_GIT_DIR) to an empty folder or an Opin checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OPIN_GIT_DIR does not look like an Opin core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Opin update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Opin..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Opin install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("opin doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("opin gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g opincore@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g opincore@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.OPIN_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.OPIN_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("opin gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("opin doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("opin gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("opin gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `opin update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `OPIN_GIT_DIR points at a non-git directory: ${gitDir}. Set OPIN_GIT_DIR (or legacy OPIN_GIT_DIR) to an empty folder or an Opin checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create an Opin git checkout at ${gitDir}? (override via OPIN_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Opin to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["opin update", "Update a source checkout (git)"],
        ["opin update --channel beta", "Switch to beta channel (git + npm)"],
        ["opin update --channel dev", "Switch to dev channel (git + npm)"],
        ["opin update --tag beta", "One-off update to a dist-tag or version"],
        ["opin update --no-restart", "Update without restarting the service"],
        ["opin update --json", "Output result as JSON"],
        ["opin update --yes", "Non-interactive (accept downgrade prompts)"],
        ["opin update wizard", "Interactive update wizard"],
        ["opin --update", "Shorthand for opin update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run opin update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.opin.squareexp.com/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opin update status", "Show channel + version status."],
          ["opin update status --json", "JSON output."],
          ["opin update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.opin.squareexp.com/cli/update")}`,
    )
    .action(async (opts) => {
      try {
