...
function appendOpinHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("opin skills list")}\` to see all skills.`
      : "No skills found.";
    return appendOpinHubHint(message, opts.json);
  }
  ...
  return appendOpinHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendOpinHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("opin skills list")}\` to see available skills.`,
      opts.json,
    );
  }
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.opin.squareexp.com/cli/skills")}\n`,
    );
