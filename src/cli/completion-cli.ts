...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Opin Completion" || line.trim() === "# Opin Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(opin completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(opin completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option("--write-state", "Write completion scripts to $OPIN_STATE_DIR/completions (no stdout)")
...
export async function installCompletion(shell: string, yes: boolean, binName = "opin") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c opin -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'opin gateway' and not 'opin other gateway', we need to check the sequence.
