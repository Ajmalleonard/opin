...
${formatHelpExamples([
  ['opin agent --to +25512345678 --message "status update"', "Start a new session."],
  ['opin agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'opin agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'opin agent --to +25512345678 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['opin agent --to +25512345678 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.opin.squareexp.com/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.opin.squareexp.com/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['opin agents set-identity --agent main --name "Opin" --emoji ""', "Set name + emoji."],
  ["opin agents set-identity --agent main --avatar avatars/opin.png", "Set avatar path."],
  [
    "opin agents set-identity --workspace ~/.opin/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "opin agents set-identity --identity-file ~/.opin/workspace/IDENTITY.md --agent main",
