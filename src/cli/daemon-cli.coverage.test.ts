...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    OPIN_STATE_DIR: process.env.OPIN_STATE_DIR,
    OPIN_CONFIG_PATH: process.env.OPIN_CONFIG_PATH,
    OPIN_GATEWAY_PORT: process.env.OPIN_GATEWAY_PORT,
    OPIN_PROFILE: process.env.OPIN_PROFILE,
  };

  beforeEach(() => {
    process.env.OPIN_STATE_DIR = "/tmp/opin-cli-state";
    process.env.OPIN_CONFIG_PATH = "/tmp/opin-cli-state/opin.json";
    delete process.env.OPIN_GATEWAY_PORT;
    delete process.env.OPIN_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.OPIN_STATE_DIR !== undefined) {
      process.env.OPIN_STATE_DIR = originalEnv.OPIN_STATE_DIR;
    } else {
      delete process.env.OPIN_STATE_DIR;
    }

    if (originalEnv.OPIN_CONFIG_PATH !== undefined) {
      process.env.OPIN_CONFIG_PATH = originalEnv.OPIN_CONFIG_PATH;
    } else {
      delete process.env.OPIN_CONFIG_PATH;
    }

    if (originalEnv.OPIN_GATEWAY_PORT !== undefined) {
      process.env.OPIN_GATEWAY_PORT = originalEnv.OPIN_GATEWAY_PORT;
    } else {
      delete process.env.OPIN_GATEWAY_PORT;
    }

    if (originalEnv.OPIN_PROFILE !== undefined) {
      process.env.OPIN_PROFILE = originalEnv.OPIN_PROFILE;
    } else {
      delete process.env.OPIN_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        OPIN_PROFILE: "dev",
        OPIN_STATE_DIR: "/tmp/opin-daemon-state",
        OPIN_CONFIG_PATH: "/tmp/opin-daemon-state/opin.json",
        OPIN_GATEWAY_PORT: "19001",
      },
