...
import type { OpinConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: OpinConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpinConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpinConfig),
...
export function getCommandDetection(_cfg?: OpinConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpinConfig) {
