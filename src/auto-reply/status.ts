...
type AgentConfig = Partial<NonNullable<NonNullable<OpinConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.opin/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
...
const formatVoiceModeLine = (config?: OpinConfig, sessionEntry?: SessionEntry): string | null => {
  if (!config) {
    return null;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 Opin ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: OpinConfig): string {
  const lines = ["ℹ️ Help", ""];
