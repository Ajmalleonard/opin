...
import { scheduleGatewaySigusr1Restart, triggerOpinRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Opin in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerOpinRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Opin via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
