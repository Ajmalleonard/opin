...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "opin-commands-"));
...
function buildParams(commandBody: string, cfg: OpinConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
