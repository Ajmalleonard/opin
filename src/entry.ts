...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.OPIN_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.OPIN_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.OPIN_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[opin] ${parsed.error}`);
    process.exit(2);
  }
