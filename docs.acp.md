...
This document describes how the Opin ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`opin acp` exposes an ACP agent over stdio and forwards prompts to a running
Opin Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Opin Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `opin acp` over stdio.

Example config:

```bash
opin config set gateway.remote.url wss://gateway-host:18789
opin config set gateway.remote.token <token>
```

Example run:

```bash
opin acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
opin acp --session agent:main:main
opin acp --session agent:design:main
opin acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Opin ACP” to start a thread.

## Execution Model

- ACP client spawns `opin acp` and speaks ACP messages over stdio.
...
```bash
opin acp --session agent:main:main
opin acp --session-label "support inbox"
opin acp --reset-session
```
...
`opin acp` resolves the Gateway URL and auth from CLI flags or config:
