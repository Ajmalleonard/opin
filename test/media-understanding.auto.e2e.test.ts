...
import type { OpinConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("opin-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("opin-bin-sherpa-");
      const modelDir = await makeTempDir("opin-sherpa-model-");
      ...
      const cfg: OpinConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("opin-bin-whispercpp-");
      const modelDir = await makeTempDir("opin-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("opin-bin-gemini-");
      ...
      const cfg: OpinConfig = { tools: { media: { image: {} } } };
