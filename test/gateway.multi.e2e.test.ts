...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `opin-e2e-${name}-`));
  const configDir = path.join(homeDir, ".opin");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPIN_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        OPIN_GATEWAY_TOKEN: inst.gatewayToken,
        OPIN_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPIN_GATEWAY_PORT: String(gwA.port),
          OPIN_GATEWAY_TOKEN: gwA.gatewayToken,
          OPIN_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPIN_GATEWAY_PORT: String(gwB.port),
          OPIN_GATEWAY_TOKEN: gwB.gatewayToken,
          OPIN_GATEWAY_PASSWORD: "",
        }),
