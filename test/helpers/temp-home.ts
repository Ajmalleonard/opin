...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OPIN_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.OPIN_STATE_DIR = path.join(base, ".opin");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "opin-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".opin", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
