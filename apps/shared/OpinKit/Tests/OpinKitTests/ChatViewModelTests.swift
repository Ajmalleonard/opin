...
private final class TestChatTransport: @unchecked Sendable, OpinChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OpinChatHistoryPayload]
    private let sessionsResponses: [OpinChatSessionsListResponse]

    private let stream: AsyncStream<OpinChatTransportEvent>
    private let continuation: AsyncStream<OpinChatTransportEvent>.Continuation

    init(
        historyResponses: [OpinChatHistoryPayload],
        sessionsResponses: [OpinChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OpinChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OpinChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OpinChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [OpinChatAttachmentPayload]) async throws -> OpinChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return OpinChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OpinChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OpinChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OpinChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = OpinChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = OpinChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { OpinChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = OpinChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = OpinChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = OpinChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { OpinChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
