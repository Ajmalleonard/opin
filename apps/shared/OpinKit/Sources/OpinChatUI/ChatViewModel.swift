...
private let chatUILogger = Logger(subsystem: "ai.opin", category: "OpinChatUI")

@MainActor
@Observable
public final class OpinChatViewModel {
    public private(set) var messages: [OpinChatMessage] = []
    ...
    public var attachments: [OpinPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OpinChatPendingToolCall] = []
    public private(set) var sessions: [OpinChatSessionEntry] = []
    private let transport: any OpinChatTransport
    ...
    private var pendingToolCallsById: [String: OpinChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OpinChatTransport) {
    ...
    public var sessionChoices: [OpinChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [OpinChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [OpinChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OpinPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OpinChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OpinChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [OpinChatMessage]) -> [OpinChatMessage] {
        var result: [OpinChatMessage] = []
    ...
    private static func dedupeKey(for message: OpinChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OpinChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> OpinChatAttachmentPayload in
            OpinChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OpinChatSessionEntry {
        OpinChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OpinChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OpinChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OpinAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OpinChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OpinPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
