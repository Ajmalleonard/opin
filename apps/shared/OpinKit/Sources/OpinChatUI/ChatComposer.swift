...
@MainActor
struct OpinChatComposer: View {
    @Bindable var viewModel: OpinChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(OpinChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(OpinChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \OpinPendingAttachment.id)
                { (att: OpinPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            OpinPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OpinChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(OpinChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(OpinChatTheme.subtleCard)
        .clipShape(Capsule())
    }
