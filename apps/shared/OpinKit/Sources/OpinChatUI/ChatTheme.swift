...
enum OpinPlatformImageFactory {
    static func image(_ image: OpinPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
