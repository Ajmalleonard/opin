...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> OpinNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> OpinNetworkStatusPayload {
        let status: OpinNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> OpinNetworkStatusPayload {
        OpinNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
