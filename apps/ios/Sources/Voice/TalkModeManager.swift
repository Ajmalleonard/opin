...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<OpinTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = OpinTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> OpinTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return OpinTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> OpinTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> OpinTalkPTTStopPayload {
    ...
    func cancelPushToTalk() async -> OpinTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = OpinTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: OpinTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: OpinAgentEventPayload.self) else {
                continue
            }
