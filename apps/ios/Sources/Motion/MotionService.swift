...
final class MotionService: MotionServicing {
    func activities(params: OpinMotionActivityParams) async throws -> OpinMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OpinMotionActivityEntry], Error>) in
        ...
        return OpinMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: OpinPedometerParams) async throws -> OpinPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<OpinPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = OpinPedometerPayload(
