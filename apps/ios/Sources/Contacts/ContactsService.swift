...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OpinContactsSearchParams) async throws -> OpinContactsSearchPayload {
        ...
        return OpinContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OpinContactsAddParams) async throws -> OpinContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OpinContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OpinContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OpinContactPayload {
        OpinContactPayload(
