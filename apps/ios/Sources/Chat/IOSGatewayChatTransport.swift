...
struct IOSGatewayChatTransport: OpinChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OpinChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OpinChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OpinChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OpinChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OpinChatAttachmentPayload]) async throws -> OpinChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OpinChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(OpinChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(OpinGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpinGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpinChatEventPayload.self)
                        {
