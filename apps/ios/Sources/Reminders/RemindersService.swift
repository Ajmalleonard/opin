...
final class RemindersService: RemindersServicing {
    func list(params: OpinRemindersListParams) async throws -> OpinRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OpinReminderPayload], Error>) in
        ...
        return OpinRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OpinRemindersAddParams) async throws -> OpinRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OpinReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OpinRemindersAddPayload(reminder: payload)
    }
