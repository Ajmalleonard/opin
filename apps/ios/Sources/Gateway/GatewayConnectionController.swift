...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [OpinCapability.canvas.rawValue, OpinCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(OpinCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(OpinCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = OpinLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(OpinCapability.location.rawValue) }

        caps.append(OpinCapability.device.rawValue)
        caps.append(OpinCapability.photos.rawValue)
        caps.append(OpinCapability.contacts.rawValue)
        caps.append(OpinCapability.calendar.rawValue)
        caps.append(OpinCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(OpinCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            OpinSystemCommand.notify.rawValue,
            OpinChatCommand.push.rawValue,
        ...
        if caps.contains(OpinCapability.camera.rawValue) {
            commands.append(OpinCameraCommand.list.rawValue)
            commands.append(OpinCameraCommand.snap.rawValue)
            commands.append(OpinCameraCommand.clip.rawValue)
        }
        if caps.contains(OpinCapability.location.rawValue) {
            commands.append(OpinLocationCommand.get.rawValue)
        }
        if caps.contains(OpinCapability.device.rawValue) {
            commands.append(OpinDeviceCommand.status.rawValue)
            commands.append(OpinDeviceCommand.info.rawValue)
        }
        if caps.contains(OpinCapability.photos.rawValue) {
            commands.append(OpinPhotosCommand.latest.rawValue)
        }
        if caps.contains(OpinCapability.contacts.rawValue) {
            commands.append(OpinContactsCommand.search.rawValue)
            commands.append(OpinContactsCommand.add.rawValue)
        }
        if caps.contains(OpinCapability.calendar.rawValue) {
            commands.append(OpinCalendarCommand.events.rawValue)
            commands.append(OpinCalendarCommand.add.rawValue)
        }
        if caps.contains(OpinCapability.reminders.rawValue) {
            commands.append(OpinRemindersCommand.list.rawValue)
            commands.append(OpinRemindersCommand.add.rawValue)
        }
        if caps.contains(OpinCapability.motion.rawValue) {
            commands.append(OpinMotionCommand.activity.rawValue)
            commands.append(OpinMotionCommand.pedometer.rawValue)
        }
