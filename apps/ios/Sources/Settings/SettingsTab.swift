...
struct SettingsTab: View {
    ...
    @AppStorage("location.enabledMode") private var locationEnabledModeRaw: String = OpinLocationMode.off.rawValue
    ...
    @State private var lastLocationModeRaw: String = OpinLocationMode.off.rawValue
    ...
    private let gatewayLogger = Logger(subsystem: "ai.opin.ios", category: "GatewaySettings")
    ...
    var body: some View {
        NavigationStack {
                ...
                Section("Device") {
                    DisclosureGroup("Features") {
                        ...
                        Picker("Location Access", selection: self.$locationEnabledModeRaw) {
                            Text("Off").tag(OpinLocationMode.off.rawValue)
                            Text("While Using").tag(OpinLocationMode.whileUsing.rawValue)
                            Text("Always").tag(OpinLocationMode.always.rawValue)
                        }
                        ...
                        Text("Keeps the screen awake while Opin is open.")
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }
            ...
            .onChange(of: self.locationEnabledModeRaw) { _, newValue in
                ...
                guard let mode = OpinLocationMode(rawValue: newValue) else { return }
                Task {
    ...
    private var locationMode: OpinLocationMode {
        OpinLocationMode(rawValue: self.locationEnabledModeRaw) ?? .off
    }
