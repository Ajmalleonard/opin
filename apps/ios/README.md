...
This is an **alpha** iOS app that connects to an Opin Gateway as a `role: node`.
...
3. On iOS: Opin → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project Opin.xcodeproj -scheme Opin -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/OpinKit` contains the shared transport/types used by the iOS app.
