...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OpinProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.opin", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OpinKit.AnyCodable]()) {
                $0[$1.key] = OpinKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OpinProtocol.AnyCodable?) -> [String: OpinProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OpinProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OpinKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OpinProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("opin.control.heartbeat")
    static let controlAgentEvent = Notification.Name("opin.control.agent")
}
