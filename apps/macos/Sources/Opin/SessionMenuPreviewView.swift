...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.opin", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpinSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpinSessionPreviewEntry,
    ...
    private static func cache(payload: OpinSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpinChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OpinKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OpinKit.AnyCodable]) -> [OpinChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpinChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpinChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpinChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpinChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpinChatMessage) -> String? {
