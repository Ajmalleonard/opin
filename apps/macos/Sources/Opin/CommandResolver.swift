...
enum CommandResolver {
    private static let projectRootDefaultsKey = "opin.gatewayProjectRootPath"
    // Preferred CLI helper names in order. Keep "opin" as a fallback for compatibility.
    private static let helperNames = ["opin", "opin"]

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let opinEntry = root.appendingPathComponent("opin.mjs").path
        if FileManager().isReadableFile(atPath: opinEntry) { return opinEntry }
        let opinEntry = root.appendingPathComponent("opin.mjs").path
        if FileManager().isReadableFile(atPath: opinEntry) { return opinEntry }
        let binEntry = root.appendingPathComponent("bin/opin.js").path
        if FileManager().isReadableFile(atPath: binEntry) { return binEntry }
        let binOpinEntry = root.appendingPathComponent("bin/opin.js").path
        if FileManager().isReadableFile(atPath: binOpinEntry) { return binOpinEntry }
        return nil
    }
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/opin")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let opinPaths = self.opinManagedPaths(home: home)
        if !opinPaths.isEmpty {
            extras.insert(contentsOf: opinPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + opinPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func opinManagedPaths(home: URL) -> [String] {
        // Support both legacy `.opin` and new `.opin` managed install locations.
        let bases = [
            home.appendingPathComponent(".opin"),
            home.appendingPathComponent(".opin"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func opinExecutable(searchPaths: [String]? = nil) -> String? {
        for name in self.helperNames {
            if let found = self.findExecutable(named: name, searchPaths: searchPaths) {
                return found
            }
        }
        return nil
    ...
    static func projectOpinExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
        let root = projectRoot ?? self.projectRoot()
        for name in self.helperNames {
            let candidate = root.appendingPathComponent("node_modules/.bin").appendingPathComponent(name).path
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("opin.mjs").path,
            root.appendingPathComponent("opin.mjs").path,
            root.appendingPathComponent("bin/opin.js").path,
            root.appendingPathComponent("bin/opin.js").path,
        ]
    ...
    static func hasAnyOpinInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.opinExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let opinPath = self.projectOpinExecutable(projectRoot: root) {
                return [opinPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                // Prefer the new `opin` package script, fall back to `opin` for compatibility.
                return [pnpm, "--silent", "opin", subcommand] + extraArgs
            }
            if let opinPath = self.opinExecutable(searchPaths: searchPaths) {
                return [opinPath, subcommand] + extraArgs
            }

            let missingEntry = """
            opin entrypoint missing (looked for dist/index.js or opin.mjs/opin.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real opin CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/opin"
        ...
        \(cliSection)
        \(projectSection)
        if command -v opin >/dev/null 2>&1; then
          CLI="$(command -v opin)"
          opin \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/opin.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/opin.mjs"
            node "$PRJ/opin.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/opin.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/opin.js"
            node "$PRJ/bin/opin.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent opin"
          pnpm --silent opin \(quotedArgs);
        else
          echo "opin CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OpinConfigFile.loadDict()
