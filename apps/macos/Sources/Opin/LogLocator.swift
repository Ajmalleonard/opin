...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPIN_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/opin")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("opin-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("opin-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/opin/ (rolling or stdout), or nil if none exist.
