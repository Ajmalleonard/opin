...
enum OpinLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = OpinOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OpinFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OpinLogging.bootstrapIfNeeded()
        let label = OpinLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OpinOSLogHandler: LogHandler {
...
struct OpinFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OpinLogging.parseLabel(self.label)
