...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.opin.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["OPIN_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "opin.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "opin.nixMode") { return true }
