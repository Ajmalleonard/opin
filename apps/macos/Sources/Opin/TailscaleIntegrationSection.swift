...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.opin/opin.json. Prefer OPIN_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.opin/opin.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.opin/opin.json. Restart the gateway to apply."
        }
