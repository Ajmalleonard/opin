...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Opin token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: OpinOAuthStore.AnthropicOAuthStatus = OpinOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.opin", category: "anthropic-oauth")
...
enum OpinOAuthStore {
    ...
    private static let opinOAuthDirEnv = "OPIN_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Opin OAuth token file not found"
            case .unreadableFile: "Opin OAuth token file not readable"
            case .invalidJSON: "Opin OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Opin OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Opin OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.opinOAuthDirEnv]?
