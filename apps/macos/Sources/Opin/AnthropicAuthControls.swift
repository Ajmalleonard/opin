...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: OpinOAuthStore.AnthropicOAuthStatus = OpinOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: OpinOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = OpinOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = OpinOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try OpinOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Opin can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: OpinOAuthStore.AnthropicOAuthStatus,
