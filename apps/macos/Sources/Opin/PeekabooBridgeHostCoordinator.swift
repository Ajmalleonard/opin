...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.opin", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OpinPeekabooBridgeServices?
    private static var opinSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Opin", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = OpinPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.opinSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.opinSocketPath, privacy: .public)")
    }
...
@MainActor
private final class OpinPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.opin.peekaboo")
