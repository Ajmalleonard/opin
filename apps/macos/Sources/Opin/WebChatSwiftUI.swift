...
private let webChatSwiftLogger = Logger(subsystem: "ai.opin", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: OpinChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OpinChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(OpinChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OpinChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OpinGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OpinChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OpinChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OpinChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: OpinChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OpinChatView>) -> NSViewController
    {
