...
struct OpinMacCLI {
    static func main() async {
        ...
        default:
            fputs("opin-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      opin-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      opin-mac discover [--timeout <ms>] [--json] [--include-local]
      opin-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      opin-mac connect
      opin-mac connect --url ws://127.0.0.1:18789 --json
      opin-mac discover --timeout 3000 --json
      opin-mac wizard --mode local
    """)
}
