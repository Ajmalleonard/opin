...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OpinProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OpinProtocol.AnyCodable] = [
            "presence": .init([OpinProtocol.AnyCodable(entry)]),
        ]
        let payload = OpinProtocol.AnyCodable(payloadMap)
