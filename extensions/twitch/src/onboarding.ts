 ...
 */
function setTwitchAccount(cfg: OpinConfig, account: Partial<TwitchAccountConfig>): OpinConfig {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: OPIN_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: OpinConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var OPIN_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.OPIN_TWITCH_ACCESS_TOKEN?.trim();
