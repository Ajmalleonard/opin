import type { OpinPluginApi, OpinPluginService } from "opin/plugin-sdk";
...
function normalizeDenyList(cfg: OpinPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OpinPluginApi["config"]): string[] {
...
export default function register(api: OpinPluginApi) {
  ...
  const timerService: OpinPluginService = {
