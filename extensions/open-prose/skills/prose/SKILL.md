...
metadata: { "opin": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Opin `sessions_spawn`
- **File I/O** == Opin `read`/`write`
- **Remote fetch** == Opin `web_fetch` (or `exec` with curl when POST is required)
