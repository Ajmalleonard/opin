import type { PluginRuntime } from "opin/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPIN_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opin-nostr-"));
  process.env.OPIN_STATE_DIR = dir;
  setNostrRuntime({
    state: {
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OPIN_STATE_DIR = previous;
    }
