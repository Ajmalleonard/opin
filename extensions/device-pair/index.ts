...
import { approveDevicePairing, listDevicePairing } from "opin/plugin-sdk";
...
function resolveGatewayPort(cfg: OpinPluginApi["config"]): number {
  const envRaw = process.env.OPIN_GATEWAY_PORT?.trim() || process.env.OPIN_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: OpinPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: OpinPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: OpinPluginApi): Promise<ResolveUrlResult> {
...
export default function register(api: OpinPluginApi) {
  api.registerCommand({
    name: "pair",
