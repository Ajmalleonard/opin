...
function resolveOpinRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPIN_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPIN_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpinRoot(), "dist", "extensionAPI.js");
