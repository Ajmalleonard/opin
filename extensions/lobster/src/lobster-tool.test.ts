...
import type { OpinPluginApi, OpinPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "opin-lobster-plugin-") {
...
function fakeApi(overrides: Partial<OpinPluginApi> = {}): OpinPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<OpinPluginToolContext> = {}): OpinPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: OpinPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
