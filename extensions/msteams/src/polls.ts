...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["opinPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["opin", "pollId"]) ??
    readNestedString(value, ["opin", "poll", "id"]) ??
    readNestedString(value, ["data", "opinPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "opin", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          ...
          value: { opinPollId: pollId, pollId },
        },
      },
    ],
  };
