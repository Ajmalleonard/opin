...
import { addWildcardAllowFrom, DEFAULT_ACCOUNT_ID, formatDocsLink } from "opin/plugin-sdk";
...
function setFeishuDmPolicy(cfg: OpinConfig, dmPolicy: DmPolicy): OpinConfig {
  const allowFrom =
    dmPolicy === "open"
...
function setFeishuAllowFrom(cfg: OpinConfig, allowFrom: string[]): OpinConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
...
function setFeishuGroupAllowFrom(cfg: OpinConfig, groupAllowFrom: string[]): OpinConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
