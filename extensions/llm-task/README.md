...
`opin.invoke --each`) without adding new Opin code per workflow.
...
This extension depends on Opin internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Opin extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.opin/extensions` as a standalone plugin directory.
