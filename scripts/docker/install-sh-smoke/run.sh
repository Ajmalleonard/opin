...
INSTALL_URL="${OPIN_INSTALL_URL:-https://opin.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPIN_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPIN_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="opin"
PACKAGE_NAME="${OPIN_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPIN_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPIN_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPIN_INSTALL_LATEST_OUT:-}"
fi
