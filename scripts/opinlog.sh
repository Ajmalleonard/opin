...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "opinlog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
opinlog - Opin Logging Utility

USAGE:
    opinlog [OPTIONS]

DESCRIPTION:
    View Opin logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Opin logs flow through the macOS unified log (subsystem: ai.opin).
...
QUICK START:
    opinlog -n 100             Show last 100 lines from all components
    opinlog -f                 Follow logs in real-time
    opinlog -e                 Show only errors
    opinlog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    opinlog                   Show last 50 lines from past 5 minutes (default)
    opinlog -f                Stream logs continuously
    opinlog -n 100            Show last 100 lines
    opinlog -e                Show only recent errors
    opinlog -l 30m -n 200     Show last 200 lines from past 30 minutes
    opinlog -c ServerManager  Show recent ServerManager logs
    opinlog -s "fail"         Search for "fail" in recent logs
    opinlog --server -e       Show recent server errors
    opinlog -f -d             Stream debug logs continuously
