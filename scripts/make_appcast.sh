...
ZIP=${1:?"Usage: $0 Opin-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/opin/opin/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Opin-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/Ajmalleonard/opin/releases/download/v${VERSION}/"}
