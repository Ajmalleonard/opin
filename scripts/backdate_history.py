...
def main():
    ...
    for i in range(1, 21):
        ...
        # Append change to pr_tracking.txt
        with open("pr_tracking.txt", "a") as f:
            f.write(f"PR {i} change by {author['username']} at {datetime.datetime.now()}\n")
            
        subprocess.run(["git", "add", "pr_tracking.txt"], capture_output=True)
