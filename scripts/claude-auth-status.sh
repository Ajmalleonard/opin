...
# Checks both Claude Code and Opin auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
OPIN_AUTH="$HOME/.opin/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    opin models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    opin_status=$(check_opin_auth 2>/dev/null || true)

    claude_expires=0
    opin_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        opin_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        opin_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$OPIN_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            opin: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$opin_status" == EXPIRED* ]] || [[ "$opin_status" == MISSING* ]]; then
        echo "OPIN_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$opin_status" == EXPIRING* ]]; then
        echo "OPIN_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Opin Auth (~/.opin/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$OPIN_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$OPIN_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'opin doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active opin >/dev/null 2>&1; then
    echo -e "Opin service: ${GREEN}running${NC}"
else
    echo -e "Opin service: ${RED}NOT running${NC}"
fi
