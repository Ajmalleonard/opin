...
# Build and bundle Opin into a minimal .app we can open.
# Outputs to dist/Opin.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Opin.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Opin"
BUNDLE_ID="${BUNDLE_ID:-ai.opin.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/opin/opin/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Opin/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpinBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpinGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Opin"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Opin"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Opin"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Opin" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Opin/Resources/Opin.icns" "$APP_ROOT/Contents/Resources/Opin.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Opin/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpinKit resources"
OPINKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpinKit_OpinKit.bundle"
if [ -d "$OPINKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpinKit_OpinKit.bundle"
  cp -R "$OPINKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpinKit_OpinKit.bundle"
else
  echo "WARN: OpinKit resource bundle not found at $OPINKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Opin"
killall -q Opin 2>/dev/null || true
