...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/opin/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Opin Re-Auth" -c "After getting key, run: ssh $SERVER '~/opin/scripts/mobile-reauth.sh'" --id opin-auth
        ;;
    *)
