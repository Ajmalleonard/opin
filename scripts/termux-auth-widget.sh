...
SERVER="${OPIN_SERVER:-${OPIN_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Opin auth..."

STATUS=$(ssh "$SERVER" '$HOME/opin/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/opin/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|OPIN_EXPIRING|OPIN_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/opin/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|OPIN_EXPIRED|OPIN_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Opin won't work until re-auth"
                ;;
        esac
        ;;
