...
#
# Suggested cron: */30 * * * * /home/admin/opin/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Opin notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., opin-alerts)
...
STATE_FILE="$HOME/.opin/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Opin if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use opin
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Opin to $NOTIFY_PHONE..."
            opin send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Opin is down. Run: ssh l36 '~/opin/scripts/mobile-reauth.sh'" "urgent"
    exit 1
