...
import type { ApiContributor, Entry, MapConfig, User } from "./update-opintributors.types.js";

const REPO = "opin/opin";
const PER_LINE = 10;

const mapPath = resolve("scripts/opintributors-map.json");
...
if (start === -1 || end === -1) {
  throw new Error("README.md missing opintributors block");
}

const next = `${readme.slice(0, start)}<p align="left">\n${block}${readme.slice(end)}`;
writeFileSync(readmePath, next);

console.log(`Updated README opintributors: ${entries.length} entries`);
