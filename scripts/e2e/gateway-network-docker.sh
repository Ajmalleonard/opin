...
IMAGE_NAME="opin-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="opin-net-e2e-$$"
GW_NAME="opin-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "OPIN_GATEWAY_TOKEN=$TOKEN" \
	  -e "OPIN_SKIP_CHANNELS=1" \
	  -e "OPIN_SKIP_GMAIL_WATCHER=1" \
	  -e "OPIN_SKIP_CRON=1" \
	  -e "OPIN_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
