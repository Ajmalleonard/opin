 ...
 * `opin update`. Use it to verify the completion installation flow
...
\`opin update\`, \`opin doctor\`, and \`opin onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Opin Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`opin completion --install\` later to enable.`));
    return;
  }
