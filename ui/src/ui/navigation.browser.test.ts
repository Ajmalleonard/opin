...
import { OpinApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = OpinApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("opin-app") as OpinApp;
...
beforeEach(() => {
  OpinApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__OPIN_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  OpinApp.prototype.connect = originalConnect;
  window.__OPIN_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/opin/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/opin");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/opin/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPIN_CONTROL_UI_BASE_PATH__ = "/opin";
    const app = mountApp("/opin/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/opin");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/opin/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("opin.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
