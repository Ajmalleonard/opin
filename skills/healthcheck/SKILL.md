---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Opin deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Opin cron scheduling for periodic checks, or version status checks on a machine running Opin (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Opin, then align it to a user-defined risk tolerance without breaking access. Use Opin security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Opin changes the host firewall, SSH, or OS updates; it does not.
...
5. Opin gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Opin can access sensitive data.
...
### 2) Run Opin security audits (read-only)

As part of the default read-only checks, run `opin security audit --deep`. Only offer alternatives if the user requests them:

1. `opin security audit` (faster, non-probing)
2. `opin security audit --json` (structured output)

Offer to apply Opin safe defaults (numbered):

1. `opin security audit --fix`

Be explicit that `--fix` only tightens Opin defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Opin version/update status (read-only)

As part of the default read-only checks, run `opin update status`.
...
- Credential hygiene notes (location of Opin creds, prefer disk encryption)
...
After Opin install or first hardening pass, run at least one baseline audit and version check:

- `opin security audit`
- `opin security audit --deep`
- `opin update status`

Ongoing monitoring is recommended. Use the Opin cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `opin cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `opin update status`
...
Before creating, `opin cron list` and match on exact `name`. If found, `opin cron edit <id> ...`.
If not found, `opin cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `opin update status` (preferred for source checkouts and channels)
2. `npm view opin version` (published npm version)

## Opin command accuracy

Use only supported commands and flags:

- `opin security audit [--deep] [--fix] [--json]`
- `opin status` / `opin status --deep`
- `opin health --json`
- `opin update status`
- `opin cron add|list|runs|run`

Do not invent CLI flags or imply Opin enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Opin compaction:
