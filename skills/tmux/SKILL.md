...
metadata: { "opin": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${OPIN_TMUX_SOCKET_DIR:-${OPIN_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/opin-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/opin.sock"
SESSION=opin-python
...
- Use `OPIN_TMUX_SOCKET_DIR` (legacy `OPIN_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$OPIN_TMUX_SOCKET_DIR/opin.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `OPIN_TMUX_SOCKET_DIR`).
